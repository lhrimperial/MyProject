...
import com.handy.frame.server.cache.CacheUtils;
import com.handy.frame.server.cache.exception.KeyIsNotFoundException;
import com.handy.frame.server.cache.exception.RedisCacheStorageException;
...
public class RedisCacheStore<K, V> implements IRemoteCacheStore<K, V>,InitializingBean,DisposableBean {
    ...
    public String set(K key, V value, int exp) {
        String skey = FastJsonUtil.toJsonString(key);
        String svalue = FastJsonUtil.toJsonString(value);
        if(skey == null) {
    ...
    public void removeMulti(K[] keys) {
        ...
        for(int i = 0,len = keys.length; i < len; i++){
            skey[i] = FastJsonUtil.toJsonString(keys[i]);
        }
        redisClient.del(skey);
    }
